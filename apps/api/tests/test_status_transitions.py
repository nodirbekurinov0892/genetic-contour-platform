...
async def _create_queued_experiment(*, cancel_requested: bool = False) -> uuid.UUID:
...
@pytest.mark.asyncio
async def test_cancel_requested_before_start_marks_cancelled():
    experiment_id = await _create_queued_experiment(cancel_requested=True)
...
@pytest.mark.asyncio
async def test_worker_completes_after_execute_job():
